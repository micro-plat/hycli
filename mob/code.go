...
func CreateCodeByCtx(c *cli.Context) (err error) {
	...
	//创建页面代码
	err = web.CreatePage(c.Args().First(), c.String("table"), webOutput)
	if err != nil {
		return err
	}

	//创建服务代码
	err = api.CreateService(c.Args().First(), c.String("table"), apiOutpath)
	if err != nil {
		return err
	}

	//创建枚举代码
	return api.CreateEnums(c.Args().First(), c.String("table"), apiOutpath)
}
