...
func NewDbIdx(unq dbIdxs, normal dbIdxs) *DbIdx {
	return &DbIdx{
		UNQ:    unq,
		Normal: normal,
	}
...
func (s dbIdxs) Len() int {
	return len(s)
}
func (s dbIdxs) SLen() int {
	return len(s) - 1
}
	...
	for i, c := range colus {
		...
		for _, idxArr := range idxs {
			...
			if v, ok := nameMap[name]; ok {
				v.Columns = append(v.Columns, c.NewIndex(idx))
			} else {
				nameMap[name] = &dbIdx{
					Name:    name,
					Columns: Columns{c.NewIndex(idx)},
				}
			}
		}
	}
